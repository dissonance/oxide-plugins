...
namespace Oxide.Plugins {
    ...
    class WipeSchedule : RustPlugin {
        ...
        [ChatCommand("lastwipe")]
        private void cmdSetWipe(BasePlayer player, string command, string[] args) {
            SendReply(player, string.Format(MSG("lastWipe", player.UserIDString), configData.LastWipe, NextWipeDays(NextWipeDate)));
        }

        [ChatCommand("nextwipe")]
        private void cmdSetWipe(BasePlayer player, string command, string[] args) {
            SendReply(player, string.Format(MSG("nextWipe", player.UserIDString), configData.LastWipe, NextWipeDays(NextWipeDate)));
        }

        [ChatCommand("wipe")]
        private void cmdSetWipe(BasePlayer player, string command, string[] args) {
            SendReply(player, string.Format(MSG("lastNextWipe", player.UserIDString), configData.LastWipe, NextWipeDays(NextWipeDate)));
        }
        #endregion
        ...
        class ConfigData {
            ...
            public string DateFormat { get; set; }
            public string LastMapWipe { get; set; }
            public string LastBPWipe { get; set; }
            public string NextMapWipe { get; set; }
            public string NextBPWipe { get; set; }
            public bool AnnounceOnJoin { get; set; }
        }
